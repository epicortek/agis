...
A verifier MUST reject a high-assurance request if the nonce has already been observed within the replay-protection window.  Nonces SHOULD be retained for at least the configured freshness window plus an allowance for clock skew.
...
* status and revocation validation with a status decision policy (`active` allows, `revoked`/`suspended`/`compromised` deny, `unknown`/`deprecated` review);
